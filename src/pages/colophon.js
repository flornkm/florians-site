import Head from "next/head";
import Link from "next/link";
import Navigation from "@/components/Navigation";
import Footer from "@/components/Footer";
import * as Icon from "react-feather";

export default function Colophon() {
  return (
    <>
      <Head>
        <title>Colophon | Design With Tech</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
        <link rel="icon" href="/favicon.ico" />
      </Head>
      <Navigation title={"Designer and Developer"} highlight={"Legal"} />
      <main class="max-md:w-[90%] w-full max-w-6xl pl-[5%] pr-[5%] m-auto bg-white">
        <div class="flex flex-col items-left justify-left h-full pt-32 max-md:pt-16 mb-16">
          <h1 class="text-2xl font-bold mb-2">Colophon</h1>
          <p class="text-base mb-10">
            A colophon describes the methods, tools, and materials used to make
            a creative work. On this site you'll therefore find information from
            where inspiration to specific contents came and which technologies I
            have used to create my space here on the internet.
          </p>
          <h2 class="text-xl font-medium mb-8">General</h2>
          <div class="flex flex-col gap-8">
            <div class="flex justify-between w-full flex-wrap gap-4">
              <h2 class="text-lg font-medium">Typography</h2>
              <div class="flex gap-4 flex-wrap max-md:gap-2">
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://fonts.google.com/specimen/Plus+Jakarta+Sans"}
                  target="_blank"
                >
                  Plus Jakarta Sans
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
              </div>
            </div>
            <div class="h-[1px] bg-gray-200 w-full mt-1 mb-1"></div>
            <div class="flex justify-between w-full flex-wrap gap-4">
              <h2 class="text-lg font-medium">Photography</h2>
              <div class="flex gap-4 flex-wrap max-md:gap-2">
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"#"}
                  target="_blank"
                >
                  Alice Sopp
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://www.marcrufeis.de/"}
                  target="_blank"
                >
                  Marc Rufeis
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
              </div>
            </div>
            <div class="h-[1px] bg-gray-200 w-full mt-1 mb-1"></div>
            <div class="flex justify-between w-full flex-wrap gap-2">
              <h2 class="text-lg font-medium">Tech Stack</h2>
              <div class="flex gap-4 flex-wrap max-md:gap-2">
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://nextjs.org/"}
                  target="_blank"
                >
                  NextJS
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://tailwindcss.com/"}
                  target="_blank"
                >
                  TailwindCSS
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://vercel.com/"}
                  target="_blank"
                >
                  Vercel
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://formspree.io/"}
                  target="_blank"
                >
                  Formspree
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
              </div>
            </div>
            <div class="h-[1px] bg-gray-200 w-full mt-1 mb-1"></div>
            <div class="flex justify-between w-full flex-wrap gap-4">
              <h2 class="text-lg font-medium">Inspiration</h2>
              <div class="flex gap-4 flex-wrap max-md:gap-2">
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://linusrogge.com/"}
                  target="_blank"
                >
                  Linus Rogge
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://www.marco.fyi/"}
                  target="_blank"
                >
                  Marco Cornacchia
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
                <Link
                  class="font-medium transition-all text-black hover:opacity-75 border-b-black group"
                  href={"https://samuelkraft.com/"}
                  target="_blank"
                >
                  Samuel Kraft
                  <Icon.ArrowUpRight
                    size={20}
                    class="inline ml-0.5 relative group-hover:-right-1 group-hover:-top-1.5 right-0 -top-0.5 transition-all"
                  />
                </Link>
              </div>
            </div>
          </div>
        </div>
        <div class="h-64"></div>
      </main>
      <Footer />
    </>
  );
}
