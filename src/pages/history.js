        ...
        <div className="flex flex-col items-left justify-left h-full pt-32 max-md:pt-16">
          ...
          <div className="flex flex-col gap-8 max-md:gap-12">
            {versions.map((version) => (
              <div className={"flex w-full justify-between items-center gap-1 max-md:flex-col-reverse max-md:items-start " + (version.version !== "1" && "border-b border-dashed border-zinc-100 pb-8 dark:border-solid dark:border-zinc-800")}>
                <div>
                  ...
                  <div className="flex items-center flex-wrap">
                    {version.changes.map((change) => (
                      <p className="text-sm text-zinc-500 dark:text-zinc-400">{change}{version.changes.indexOf(change) !== version.changes.length-1 && <span className="mx-1">-</span>}</p>
                    ))}
                  </div>
                </div>
                <div>
