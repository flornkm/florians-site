...
export function scoreTwitterTweet(el: Element): TextScore | null {
  ...
  if (/^@[a-z]+\d{4,}$/i.test(handle) || /bot$/i.test(handle)) {
    score.rating = Math.min(1, score.rating + 0.1);
...
export async function scoreVideoElement(el: HTMLVideoElement): Promise<ImageScore> {
  const signals: string[] = [];
  let score = 0;

  const candidates = [el.currentSrc, el.src, el.querySelector("source")?.src, el.poster].filter(
    Boolean,
  ) as string[];
...
export async function scoreIframeEmbed(el: HTMLIFrameElement): Promise<ImageScore> {
  const signals: string[] = [];
  let score = 0;
  const src = el.src || "";
